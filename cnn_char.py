...
class CharCNN:
    ...
    def train(self, train_data_dir, save_model_path):
    ...
    def list_all_files(self, root):
        ...
        for i in range(len(list_all)):
            element = os.path.join(root, list_all[i])
    ...
    def init_data(self, init_dir):
        ...
        for file in files:
            ...
            x.append(resize_img)
            # 获取图片文件全目录
            init_dir = os.path.dirname(file)
            # 获取图片文件上一级目录名
            dir_name = os.path.split(init_dir)[-1]
            vector_y = [0 for _ in range(len(self.dataset))]
...
if __name__ == '__main__':
    data_dir = os.path.join('data/train/cnn_char_train')
    test_dir = os.path.join('data/train/cnn_char_test')
    train_model_path = os.path.join('./data/model/char_recognize/model.ckpt')
    model_path = os.path.join('./data/model/char_recognize/model.ckpt-660')
